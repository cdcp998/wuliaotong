"""供应商名称归一（P9-P1③，纯本地规则不用 LLM）：送货单识别供应商名与库中供应商别名/
简写/错字判断为同一实体，复用已有供应商避免重复建档。未命中返回 (0, "")（维持精确匹配现状）。
...
def match_supplier_local(db: Session, name: str) -> tuple[int, str]:
    """本地供应商别名归一：识别名与库中启用供应商归一后
