/** 操作日志详情抽屉 UI 验证：登录 → /system/logs → 点击首行 → 截图。 */
...
async function main() {
  const profile = join(tmpdir(), `wlt-logui-${Date.now()}`);
  ...
  await sleep(3000);
  await cdp.send("Page.navigate", { url: BASE + "/system/logs" });
  await sleep(3000);
  // 点击第一行（打开抽屉）
  await cdp.eval(`(() => { const row = document.querySelector('.ant-table-tbody .ant-table-row'); if (row) row.click(); })()`);
  await sleep(1200);
  const shot = await cdp.send("Page.captureScreenshot", { format: "png" });
  writeFileSync("G:/wuliaotong_dev/AI开发文档/screenshots/b5_log_diff_drawer.png", Buffer.from(shot.data, "base64"));
  console.log("drawer open:", await cdp.eval(`!!document.querySelector('.ant-drawer-open')`), "| saved b5_log_diff_drawer.png");
