...
CREATE TABLE IF NOT EXISTS cable_fault (
  ...
  lat                 DECIMAL(10,7) NULL COMMENT '纬度（可空=尚未标记，维修寻找/后台标记后补）',
  lng                 DECIMAL(10,7) NULL COMMENT '经度（可空=尚未标记）',
  cumulative_distance DECIMAL(12,2) NOT NULL DEFAULT 0,
  fault_type          VARCHAR(30) NOT NULL DEFAULT '',
  severity            TINYINT NOT NULL DEFAULT 1 COMMENT '1 低 / 2 中 / 3 高',
  description         VARCHAR(500) NOT NULL DEFAULT '',
  status              TINYINT NOT NULL DEFAULT 0 COMMENT '0 待处理 / 1 已派发(legacy) / 2 进行中 / 3 待审核 / 4 已完成 / 5 已关闭（由关联任务自动同步）',
