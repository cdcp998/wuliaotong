...
> FastAPI + Python 3.13 + MySQL；依据《需求大纲.md》《数据库设计.md》（2026-07-28 确认版）
...
5. **外网安全**：正式 HTTPS 证书；Session Cookie HttpOnly+Secure+SameSite=Lax；登录连续失败 3 次/10 分钟要求 4 位数字+字母验证码（4007）；改密/重置后其他会话失效；操作日志留存；**全局限流防刷屏（见 §12）**。
6. **定时任务**：APScheduler —— 库存预警扫描（每分钟）、每日凌晨自动备份（mysqldump → sys_backup_log）、会话过期清理。
7. **Excel**：openpyxl 导入导出（商品/供应商/期初/报表），导入返回逐行错误信息。
8. **错误码**：4001 库存不足 / 4002 单据状态不允许 / 4003 商品或库位不存在 / 4004 登录失败或已锁定 / 4005 无权限 / 4006 参数校验失败 / 4007 需要验证码 / **4008 请求过于频繁（限流，HTTP 429）** / 5001 OCR 引擎未初始化 / 5002 大模型调用失败 / 5003 文件处理失败。
...
12. **接口限流（反刷屏/反垃圾）+ Windows Proactor 加固**（2026-08-08）：全局 `RateLimitMiddleware`（`app/core/ratelimit.py`）——每 IP 滑动窗口计数（默认 300 次/60 秒，环境变量 `RATE_LIMIT_ENABLED`/`RATE_LIMIT_REQUESTS`/`RATE_LIMIT_WINDOW_SECONDS` 可调）；执行顺序 CORS → 限流 → 审计：被限请求直接返回 HTTP 429 + 统一响应体 `code=4008`「请求过于频繁，请稍后再试」+ `Retry-After` + `Connection: close`（带 CORS 头、**不写操作日志**，避免洪泛放大审计 DB 写）；`/health` 豁免（运维探活不受影响）；单进程内存态（与验证码/重置码一致），重启清零。**Windows 已知问题兜底**（`app/core/loop_guard.py`，lifespan 安装，幂等）：① Python 3.13 Proactor 下客户端强制断开时 `_call_connection_lost()` 内 `sock.shutdown()` 未捕获 OSError（WinError 10054/10053/64 等）冒泡为 asyncio 未处理异常——安装事件循环异常过滤器，仅静默该良性模式（OSError 且 winerror/errno ∈ {64,10053,10054,995,996,1236}，回调含 `_call_connection_lost`），其余异常链到原处理器；② 客户端在 accept 完成前断开时 stdlib 的 `BaseProactorEventLoop._start_serving` 会**关闭监听 socket**（服务停止接受新连接）且 `accept_coro` 产生「Task exception was never retrieved」——按本机 3.13.8 源码逐字复制 + 单一改动打补丁：良性 accept 错误改为「50ms 退避后重挂 accept」、`IocpProactor.accept` 的 `accept_coro` 吞掉 OSError；补丁安装前校验源码标记（版本漂移自动跳过并告警，不影响限流与过滤器）。
