/** 设备维修任务三模式派发 UI 验证：打开新建弹窗截图 + 列表截图。 */
...
async function main() {
  const profile = join(tmpdir(), `wlt-dm-${Date.now()}`);
  ...
  await sleep(3000);
  await cdp.send("Page.navigate", { url: BASE + "/device/tasks" });
  await sleep(3000);
  // 打开新建弹窗
  await cdp.eval(`(() => { [...document.querySelectorAll('button')].find(b=>b.textContent.includes('新建设备维修'))?.click(); })()`);
  await sleep(1000);
  const shot = await cdp.send("Page.captureScreenshot", { format: "png" });
  writeFileSync("G:/wuliaotong_dev/AI开发文档/screenshots/b5_device_dispatch_modal.png", Buffer.from(shot.data, "base64"));
  console.log("modal:", await cdp.eval(`!!document.querySelector('.ant-modal-open')`), "| saved b5_device_dispatch_modal.png");
