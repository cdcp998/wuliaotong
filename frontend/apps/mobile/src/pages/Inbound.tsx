    ...
    <div style={{ minHeight: "100vh", background: "#f5f6f8" }}>
      ...
      <Popup visible={locPicker.open} onMaskClick={() => setLocPicker((s) => ({ ...s, open: false }))} bodyStyle={{ height: "50vh" }}>
        <List header="选择库位">
          {locPicker.locations.map((l) => (
            <List.Item key={l.id} onClick={() => { updateRow(locPicker.rowIndex, { location: l }); setLocPicker((s) => ({ ...s, open: false })); }}>{l.code}</List.Item>
          ))}
      ...
      <Popup visible={!!ocrConfirm} onMaskClick={cancelOcr} bodyStyle={{ height: "auto", maxHeight: "80vh" }}>
        <div style={{ padding: 16 }}>
          ...
          <p style={{ color: "#999", fontSize: 12, margin: "0 0 12px" }}>识别内容可修改；确认后供应商落库、物料自动匹配/新增并带入明细</p>
          <List>
          ...
          <div style={{ marginTop: 10, fontSize: 13 }}>
            ...
            <div style={{ maxHeight: 220, overflowY: "auto" }}>
              ...
              {!ocrConfirm?.items.length && <div style={{ color: "#999", padding: "8px 0" }}>未识别到明细条目，可确认后手动添加材料</div>}
            </div>
          </div>
      ...
      <Popup visible={newMaterial.open} onMaskClick={() => setNewMaterial((s) => ({ ...s, open: false }))} bodyStyle={{ height: "auto" }}>
        <div style={{ padding: 16 }}>
          ...
          <p style={{ color: "#999", fontSize: 12, margin: "0 0 12px" }}>未匹配到该条码对应的材料，确认信息后新增（条码可选）</p>
          <List>
            <List.Item extra={
