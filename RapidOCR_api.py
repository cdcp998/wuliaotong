...
class OcrAPI:
    ...
    def __init__(self, argsStr = ""):
        ...
        if 'ensureAscii' not in args:
            args += f'--ensureAscii=0 --models=models ' \
                    f'--det=ch_PP-OCRv4_det_infer.onnx --rec=rec_ch_PP-OCRv4_infer.onnx ' \
                    f'--doAngle=0 mostAngle=0'
    ...
    def printResult(res: dict):
        ...
        if res["code"] == 100:
            ...
            for line in res["data"]:
                print(f"{index}-置信度：{round(line['score'], 2)}，文本：{line['text']}，坐標：{line['box']}")
                index += 1
        elif res["code"] == 100:
            print("图片中未识别出文字。")
        else:
