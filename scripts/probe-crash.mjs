/** 页面崩溃诊断：登录后打开指定页面，收集 console 错误/异常 + 最终 location。
 * 用法：node scripts/probe-crash.mjs /purchase-in
 */
...
const TARGET = process.argv[2] || "/purchase-in";
...
async function main() {
  const profile = join(tmpdir(), `wlt-crash-${Date.now()}`);
  ...
  const errs = cdp.events.filter((e) => ["Runtime.exceptionThrown", "Runtime.consoleAPICalled", "Log.entryAdded"].includes(e.method));
  for (const e of errs.slice(0, 20)) {
    if (e.method === "Runtime.exceptionThrown") {
      const d = e.params?.exceptionDetails;
      console.log("[EXC]", d?.text, d?.exception?.description?.slice(0, 400), d?.stackTrace?.callFrames?.slice(0, 3).map((f) => `${f.url}:${f.lineNumber}`).join(" | "));
    } else if (e.method === "Runtime.consoleAPICalled" && ["error", "warning"].includes(e.params?.type)) {
      console.log("[CONSOLE." + e.params.type + "]", e.params?.args?.map((a) => a.value ?? a.description ?? "").join(" ").slice(0, 400));
    } else if (e.method === "Log.entryAdded" && e.params?.entry?.level === "error") {
      console.log("[LOG]", e.params.entry.text?.slice(0, 400));
    }
  }
