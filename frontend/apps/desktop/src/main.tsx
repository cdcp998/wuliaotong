...
// map 模块页面（地图工作台 + 地图缓存管理；依赖 cable 模块）
const MapWorkbenchPage = lazy(() => import("./modules/map/MapWorkbench").then((m) => ({ default: m.MapWorkbenchPage })));
const MapCachePage = lazy(() => import("./modules/map/MapCache").then((m) => ({ default: m.MapCachePage })));
// task 模块页面
...
const router = createBrowserRouter(
  ...
  { path: "/cable/cache", element: withLayout(<RequireModule code="map"><MapCachePage /></RequireModule>) },
  // task 模块
...
// HMR/重复执行时复用同一 root，避免「createRoot() on a container that has already been passed to createRoot()」告警
type RootHolder = HTMLDivElement & { __wltRoot?: ReactDOM.Root };
const holder = document.getElementById("root") as RootHolder;
const root = holder.__wltRoot ?? ReactDOM.createRoot(holder);
holder.__wltRoot = root;
root.render(
  <React.StrictMode>
    <ConfigProvider
      locale={zhCN}
